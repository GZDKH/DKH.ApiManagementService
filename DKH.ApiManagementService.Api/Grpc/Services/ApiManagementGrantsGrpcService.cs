...
/// <summary>
/// gRPC server-side implementation of ResourceAccessGrantsService for ApiManagementService.
/// Restricts resource_type to "api_key" / "ai_provider" — other resource types are managed by their own services.
/// </summary>
public sealed class ApiManagementGrantsGrpcService(
...
{
    private static readonly string[] AllowedResourceTypes = ["api_key", "ai_provider"];
    ...
    private static void ValidateResourceType(string resourceType)
    {
        if (!AllowedResourceTypes.Contains(resourceType))
        {
            throw new RpcException(new Status(StatusCode.InvalidArgument,
                $"Resource type '{resourceType}' is not managed by ApiManagementService. Allowed: {string.Join(", ", AllowedResourceTypes)}."));
        }
    }
